...
from nvlx.http_v166666666633112345675678512344563 import (
    HealthServer,
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for Grpc-Message-Type Connection-nomination requests")


class GrpcMessageTypeConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            (None, None),
            ("Grpc-Message-Type", "example.v1.Request"),
            ("Connection", "grpc-message-type-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_grpc_message_type_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            "Grpc-Message-Type",
            "GrPc-MeSsAgE-TyPe",
            "keep-alive, grpc-message-type",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_grpc_message_type_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_grpc_message_type_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "grpc-accept-encoding"
        self.assertFalse(_request_grpc_message_type_nomination_is_safe(headers))
    ...
    def test_grpc_accept_encoding_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nConnection: grpc-accept-encoding, grpc-message-type\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
