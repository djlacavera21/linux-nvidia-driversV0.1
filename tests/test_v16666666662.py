...
from nvlx.http_v16666666662 import (
    HealthServer,
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for critical Connection nomination")


class ConnectionCriticalNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_critical_option_set_is_exact(self):
        self.assertEqual(
            _CRITICAL_CONNECTION_OPTIONS,
            frozenset(
                {
    ...
    def test_helper_accepts_absent_lifecycle_and_custom_options(self):
        for value in (
            None,
            "close",
            "keep-alive",
            "close, close",
            "x-custom",
            "x-host, content-length-x",
        ):
            with self.subTest(value=value):
                headers = Message()
                if value is not None:
                    headers["Connection"] = value
                self.assertTrue(_connection_critical_nomination_is_safe(headers))

        multiple = Message()
        multiple["Connection"] = "keep-alive"
        multiple["Connection"] = "x-custom"
        self.assertTrue(_connection_critical_nomination_is_safe(multiple))

    def test_helper_rejects_each_critical_option_case_insensitively(self):
        for value in (
            "Connection",
            "HOST",
            "Content-Length",
            ...
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_connection_critical_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contracts(self):
        for value in ("", "close,", "close, keep-alive"):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_connection_critical_nomination_is_safe(headers))
        self.assertFalse(_connection_critical_nomination_is_safe({}))

    def test_custom_connection_options_remain_accepted(self):
        ...
        try:
            response = self.raw_request(
                ...
                b"Host: localhost\r\n"
                b"Connection: x-host, content-length-x\r\n\r\n",
            )
    ...
    def test_host_nomination_is_terminal_400_before_runtime(self):
    ...
    def test_content_length_zero_cannot_be_nominated_hop_by_hop(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_other_critical_nominations_are_rejected(self):
        for option in (b"Transfer-Encoding", b"Trailer", b"Expect", b"Connection"):
    ...
    def test_http_10_host_nomination_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_lifecycle_gate_remains_earlier(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\n"
                b"Host: localhost\r\n"
                b"Connection: close, keep-alive, Host\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
    ...
    def test_critical_nomination_rejection_head_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_critical_nomination_rejection_terminates_pipeline(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_critical_nomination_releases_capacity_and_recovers(self):
        server = HealthServer(
