...
from nvlx.http_v16666666663311234567567851234451 import (
    HealthServer,
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for X-Envoy-Peer-Metadata-ID Connection-nomination requests")


class XEnvoyPeerMetadataIdConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            (None, None),
            ("X-Envoy-Peer-Metadata-ID", "peer-id-1"),
            ("Connection", "x-envoy-peer-metadata-id-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_x_envoy_peer_metadata_id_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            "X-Envoy-Peer-Metadata-ID",
            "X-EnVoY-PeEr-MeTaDaTa-Id",
            "keep-alive, x-envoy-peer-metadata-id",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_x_envoy_peer_metadata_id_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_x_envoy_peer_metadata_id_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "x-envoy-peer-metadata"
        self.assertFalse(_request_x_envoy_peer_metadata_id_nomination_is_safe(headers))
    ...
    def test_nomination_is_terminal_400_before_runtime(self):
        ...
        try:
            response = self.raw_request(
                server,
                b"GET /metrics HTTP/1.1\r\nHost: localhost\r\nConnection: x-envoy-peer-metadata-id\r\n\r\n",
            )
    ...
    def test_peer_metadata_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nConnection: x-envoy-peer-metadata, x-envoy-peer-metadata-id\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
