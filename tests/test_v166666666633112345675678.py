...
from nvlx.http_v166666666633112345675678 import (
    HealthServer,
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for X-Request-ID-nomination requests")


class XRequestIdConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            (None, None),
            ("X-Request-ID", "4f8c6f76-5b91-4f0f-b3b8-5efc2e2b9e0f"),
            ("Connection", "x-request-id-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_x_request_id_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            "X-Request-ID",
            "X-ReQuEsT-Id",
            "keep-alive, x-request-id",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_x_request_id_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_x_request_id_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "x-forwarded-client-cert"
        self.assertFalse(_request_x_request_id_nomination_is_safe(headers))
    ...
    def test_ordinary_header_remains_accepted(self):
        ...
        try:
            response = self.raw_request(
                ...
                b"X-Request-ID: 4f8c6f76-5b91-4f0f-b3b8-5efc2e2b9e0f\r\n\r\n",
            )
    ...
    def test_xfcc_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\n"
                b"Connection: x-forwarded-client-cert, x-request-id\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
