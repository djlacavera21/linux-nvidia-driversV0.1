import http.client
import io
import socket
import time
import unittest

from nvlx.http_v16666661 import (
    HealthServer,
    _HeaderFieldCountReader,
    _validate_request_header_fields,
)


class _MetricsRuntime:
    def __init__(self):
        self.calls = 0

    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for excess header fields")


class RequestHeaderFieldBudgetTests(unittest.TestCase):
    @staticmethod
    def raw_request(server, payload: bytes) -> bytes:
        with socket.create_connection(
            ("127.0.0.1", server.httpd.server_port), timeout=2
        ) as sock:
            sock.settimeout(2)
            sock.sendall(payload)
            sock.shutdown(socket.SHUT_WR)
            response = b""
            while True:
                chunk = sock.recv(65536)
                if not chunk:
                    break
                response += chunk
            return response

    @staticmethod
    def wait_for_slots(server, expected: int, timeout: float = 1.5) -> None:
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
            if getattr(server._request_slots, "_value", None) == expected:
                return
            time.sleep(0.005)
        raise AssertionError(
            f"request slot value did not become {expected}; "
            f"got {getattr(server._request_slots, '_value', None)}"
        )

    @staticmethod
    def header_block(field_count: int) -> bytes:
        lines = [b"Host: x\r\n"]
        for index in range(field_count - 1):
            lines.append(f"X-{index}: v\r\n".encode("ascii"))
        return b"".join(lines) + b"\r\n"

    def test_header_field_limit_validation_is_strict_and_bounded(self):
        self.assertEqual(_validate_request_header_fields(1), 1)
        self.assertEqual(_validate_request_header_fields(32), 32)
        self.assertEqual(_validate_request_header_fields(100), 100)
        with self.assertRaises(TypeError):
            _validate_request_header_fields(True)
        with self.assertRaises(TypeError):
            _validate_request_header_fields(32.0)
        with self.assertRaises(TypeError):
            _validate_request_header_fields("32")
        with self.assertRaises(ValueError):
            _validate_request_header_fields(0)
        with self.assertRaises(ValueError):
            _validate_request_header_fields(-1)
        with self.assertRaises(ValueError):
            _validate_request_header_fields(101)

    def test_reader_counts_field_starts_not_continuations(self):
        data = (
            b"A: one\r\n"
            b" two\r\n"
            b"\tthree\r\n"
            b"B: four\r\n"
            b"\r\n"
        )
        reader = _HeaderFieldCountReader(io.BytesIO(data), 2)
        self.assertEqual(reader.readline(), b"A: one\r\n")
        self.assertEqual(reader.readline(), b" two\r\n")
        self.assertEqual(reader.readline(), b"\tthree\r\n")
        self.assertEqual(reader.readline(), b"B: four\r\n")
        self.assertEqual(reader.fields, 2)
        self.assertEqual(reader.readline(), b"\r\n")

    def test_exact_field_budget_is_accepted(self):
        server = HealthServer(
            object(),
            "127.0.0.1",
            0,
            max_request_header_fields=32,
        ).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\n" + self.header_block(32),
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 200 OK\r\n"))
            self.assertTrue(response.endswith(b"ok\n"))
        finally:
            server.close()

    def test_excess_fields_use_canonical_431_before_runtime(self):
        runtime = _MetricsRuntime()
        server = HealthServer(
            runtime,
            "127.0.0.1",
            0,
            max_request_header_fields=4,
        ).start()
        try:
            response = self.raw_request(
                server,
                b"GET /metrics HTTP/1.1\r\n" + self.header_block(5),
            )
            self.assertTrue(
                response.startswith(b"HTTP/1.0 431 Request Rejected\r\n")
            )
            self.assertIn(b"Server: nvlx\r\n", response)
            self.assertIn(b"Cache-Control: no-store\r\n", response)
            self.assertIn(b"Connection: close\r\n", response)
            self.assertTrue(response.endswith(b"request rejected\n"))
            self.assertEqual(runtime.calls, 0)
        finally:
            server.close()

    def test_excess_head_fields_are_bodyless_431(self):
        server = HealthServer(
            object(),
            "127.0.0.1",
            0,
            max_request_header_fields=2,
        ).start()
        try:
            response = self.raw_request(
                server,
                b"HEAD /livez HTTP/1.1\r\n" + self.header_block(3),
            )
            head, body = response.split(b"\r\n\r\n", 1)
            self.assertTrue(head.startswith(b"HTTP/1.0 431 Request Rejected\r\n"))
            self.assertIn(b"Content-Length: 17\r\n", head + b"\r\n")
            self.assertIn(b"Connection: close\r\n", head + b"\r\n")
            self.assertEqual(body, b"")
        finally:
            server.close()

    def test_field_rejection_releases_capacity_and_server_recovers(self):
        server = HealthServer(
            object(),
            "127.0.0.1",
            0,
            max_concurrent_requests=1,
            max_request_header_fields=2,
        ).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\n" + self.header_block(3),
            )
            self.assertTrue(
                response.startswith(b"HTTP/1.0 431 Request Rejected\r\n")
            )
            self.wait_for_slots(server, 1)

            connection = http.client.HTTPConnection(
                "127.0.0.1", server.httpd.server_port, timeout=2
            )
            try:
                connection.request("GET", "/livez")
                live = connection.getresponse()
                self.assertEqual(live.status, 200)
                self.assertEqual(live.read(), b"ok\n")
            finally:
                connection.close()
        finally:
            server.close()

    def test_field_and_byte_budgets_are_independent(self):
        server = HealthServer(
            object(),
            "127.0.0.1",
            0,
            max_request_header_fields=2,
            max_request_header_bytes=512,
        ).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\n"
                b"Host: x\r\n"
                b"X-Pad: " + (b"a" * 300) + b"\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 200 OK\r\n"))
            self.assertTrue(response.endswith(b"ok\n"))
        finally:
            server.close()

    def test_defaults_and_inherited_ingress_controls_are_exposed(self):
        server = HealthServer(object(), "127.0.0.1", 0)
        try:
            self.assertEqual(server.max_request_header_fields, 32)
            self.assertEqual(server.max_request_line_bytes, 8192)
            self.assertEqual(server.max_request_header_bytes, 32768)
            self.assertEqual(server.request_header_deadline_seconds, 5.0)
            self.assertEqual(server.request_timeout_seconds, 5.0)
            self.assertEqual(server.max_concurrent_requests, 32)
        finally:
            server.httpd.server_close()


if __name__ == "__main__":
    unittest.main()
