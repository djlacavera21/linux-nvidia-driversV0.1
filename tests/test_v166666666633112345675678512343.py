...
from nvlx.http_v166666666633112345675678512343 import (
    HealthServer,
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for User-Agent Connection-nomination requests")


class UserAgentConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            (None, None),
            ("User-Agent", "nvlx-test/1.0"),
            ("Connection", "user-agent-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_user_agent_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            with self.subTest(value=value):
                ...
                self.assertFalse(_request_user_agent_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_user_agent_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "referer"
        self.assertFalse(_request_user_agent_nomination_is_safe(headers))
    ...
    def test_referer_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
