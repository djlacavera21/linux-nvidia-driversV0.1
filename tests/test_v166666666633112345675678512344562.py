...
from nvlx.http_v166666666633112345675678512344562 import (
    HealthServer,
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for Grpc-Accept-Encoding Connection-nomination requests")


class GrpcAcceptEncodingConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            ...
            ("Connection", "grpc-accept-encoding-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_grpc_accept_encoding_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            "Grpc-Accept-Encoding",
            "GrPc-AcCePt-EnCoDiNg",
            "keep-alive, grpc-accept-encoding",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_grpc_accept_encoding_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_grpc_accept_encoding_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "grpc-encoding"
        self.assertFalse(_request_grpc_accept_encoding_nomination_is_safe(headers))
    ...
    def test_nomination_is_terminal_400_before_runtime(self):
        ...
        try:
            response = self.raw_request(
                server,
                b"GET /metrics HTTP/1.1\r\nHost: localhost\r\nConnection: grpc-accept-encoding\r\n\r\n",
            )
    ...
    def test_grpc_encoding_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nConnection: grpc-encoding, grpc-accept-encoding\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
