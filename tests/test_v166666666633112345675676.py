...
from nvlx.http_v166666666633112345675676 import (
    HealthServer,
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for X-Envoy-IP-Tags-nomination requests")


class XEnvoyIpTagsConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            (None, None),
            ("X-Envoy-IP-Tags", "trusted-edge"),
            ("Connection", "x-envoy-ip-tags-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_x_envoy_ip_tags_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            "X-Envoy-IP-Tags",
            "X-EnVoY-Ip-TaGs",
            "keep-alive, x-envoy-ip-tags",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_x_envoy_ip_tags_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_x_envoy_ip_tags_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "x-envoy-force-trace"
        self.assertFalse(_request_x_envoy_ip_tags_nomination_is_safe(headers))
    ...
    def test_force_trace_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\n"
                b"Connection: x-envoy-force-trace, x-envoy-ip-tags\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
