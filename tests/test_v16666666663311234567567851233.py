...
from nvlx.http_v16666666663311234567567851233 import (
    HealthServer,
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for Traceparent-nomination requests")


class TraceparentConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_traceparent_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            "Traceparent",
            "TrAcEpArEnT",
            "keep-alive, traceparent",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_traceparent_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_traceparent_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "x-amzn-trace-id"
        self.assertFalse(_request_traceparent_nomination_is_safe(headers))
    ...
    def test_x_amzn_trace_id_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nConnection: x-amzn-trace-id, traceparent\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
