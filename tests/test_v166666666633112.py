...
from nvlx.http_v166666666633112 import (
    HealthServer,
    _request_authorization_nomination_is_safe,
)
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for Authorization Connection nomination")


class AuthorizationNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_origin_authorization_and_unrelated_options(self):
        headers = Message()
        headers["Authorization"] = "Bearer abc"
        headers["Connection"] = "keep-alive, authorization-x"
        self.assertTrue(_request_authorization_nomination_is_safe(headers))

    def test_helper_rejects_exact_authorization_nomination_case_insensitively(self):
        for value in ("authorization", "Authorization", "keep-alive, AuThOrIzAtIoN"):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_authorization_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contracts(self):
        self.assertFalse(_request_authorization_nomination_is_safe({}))
        headers = Message()
    ...
    def test_authorization_nomination_is_terminal_400_before_runtime(self):
    ...
    def test_case_folded_nomination_rejected_and_lookalike_accepted(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            ...
            accepted = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nConnection: authorization-x\r\n\r\n",
            )
            self.assertTrue(accepted.startswith(b"HTTP/1.0 200 OK\r\n"))
        finally:
            server.close()

    def test_http_10_authorization_nomination_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_proxy_authorization_gate_remains_earlier(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_authorization_nomination_rejection_head_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_authorization_nomination_rejection_terminates_pipeline(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_authorization_nomination_releases_capacity_and_recovers(self):
