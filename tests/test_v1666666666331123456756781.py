...
from nvlx.http_v1666666666331123456756781 import (
    HealthServer,
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for X-Client-Trace-ID-nomination requests")


class XClientTraceIdConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            (None, None),
            ("X-Client-Trace-ID", "a2f47c2d-b5cb-4d1d-a93f-3fbb753dcf70"),
            ("Connection", "x-client-trace-id-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_x_client_trace_id_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            "X-Client-Trace-ID",
            "X-ClIeNt-TrAcE-Id",
            "keep-alive, x-client-trace-id",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_x_client_trace_id_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_x_client_trace_id_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "x-request-id"
        self.assertFalse(_request_x_client_trace_id_nomination_is_safe(headers))
    ...
    def test_ordinary_header_remains_accepted(self):
        ...
        try:
            response = self.raw_request(
                ...
                b"X-Client-Trace-ID: a2f47c2d-b5cb-4d1d-a93f-3fbb753dcf70\r\n\r\n",
            )
    ...
    def test_expect_gate_retains_precedence(self):
        ...
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nExpect: 100-continue\r\n"
                b"Connection: x-client-trace-id\r\n\r\n",
            )
    ...
    def test_request_id_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\n"
                b"Connection: x-request-id, x-client-trace-id\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
