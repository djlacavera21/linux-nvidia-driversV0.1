...
from nvlx.http_v16666666663311234567566 import (
    HealthServer,
    _request_x_envoy_upstream_rq_timeout_alt_response_nomination_is_safe,
)
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for X-Envoy-Upstream-Rq-Timeout-Alt-Response-nomination requests")


class XEnvoyUpstreamRqTimeoutAltResponseConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            ...
            ("Connection", "x-envoy-upstream-rq-timeout-alt-response-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_x_envoy_upstream_rq_timeout_alt_response_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            "X-Envoy-Upstream-Rq-Timeout-Alt-Response",
            "X-EnVoY-UpStReAm-Rq-TiMeOuT-AlT-ReSpOnSe",
            "keep-alive, x-envoy-upstream-rq-timeout-alt-response",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_x_envoy_upstream_rq_timeout_alt_response_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_x_envoy_upstream_rq_timeout_alt_response_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "x-envoy-upstream-alt-stat-name"
        self.assertFalse(_request_x_envoy_upstream_rq_timeout_alt_response_nomination_is_safe(headers))
    ...
    def test_nomination_is_terminal_400_before_runtime(self):
        ...
        try:
            response = self.raw_request(
                server,
                b"GET /metrics HTTP/1.1\r\nHost: localhost\r\nConnection: x-envoy-upstream-rq-timeout-alt-response\r\n\r\n",
            )
    ...
    def test_expect_gate_retains_precedence(self):
        ...
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nExpect: 100-continue\r\n"
                b"Connection: x-envoy-upstream-rq-timeout-alt-response\r\n\r\n",
            )
    ...
    def test_upstream_alt_stat_name_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\n"
                b"Connection: x-envoy-upstream-alt-stat-name, x-envoy-upstream-rq-timeout-alt-response\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
