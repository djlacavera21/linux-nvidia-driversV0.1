...
from nvlx.http_v1666666663 import (
    HealthServer,
    _request_trailer_declaration_is_safe,
)
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for rejected Trailer declaration")


class TrailerContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_absent_trailer_declaration(self):
        headers = Message()
        self.assertTrue(_request_trailer_declaration_is_safe(headers))
        headers["Host"] = "localhost"
        headers["TE"] = "trailers"
        self.assertTrue(_request_trailer_declaration_is_safe(headers))

    def test_helper_rejects_single_empty_and_duplicate_trailer_fields(self):
        for value in ("X-Checksum", "Digest", ""):
            with self.subTest(value=value):
                headers = Message()
                headers["Trailer"] = value
                self.assertFalse(_request_trailer_declaration_is_safe(headers))

        duplicate = Message()
        duplicate["Trailer"] = "X-One"
        duplicate["Trailer"] = "X-Two"
        self.assertFalse(_request_trailer_declaration_is_safe(duplicate))
        self.assertFalse(_request_trailer_declaration_is_safe({}))
        self.assertFalse(_request_trailer_declaration_is_safe(None))

    def test_no_trailer_and_te_trailers_remain_accepted(self):
        for extra in (b"", b"TE: trailers\r\n"):
    ...
    def test_trailer_field_is_terminal_400_before_runtime(self):
    ...
    def test_empty_trailer_field_is_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_duplicate_trailer_fields_are_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_http_10_trailer_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_expect_gate_retains_precedence_over_trailer_gate(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_trailer_rejection_head_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_trailer_rejection_terminates_pipelined_bytes(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_trailer_rejection_releases_capacity_and_recovers(self):
        server = HealthServer(
