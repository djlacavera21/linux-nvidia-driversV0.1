...
from nvlx.http_v1666666666 import HealthServer, _connection_token_list_is_safe
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for malformed Connection syntax")


class ConnectionTokenListContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_absent_and_canonical_token_lists(self):
        values = (
            None,
            "close",
            "keep-alive",
        ...
        for value in values:
            with self.subTest(value=value):
                ...
                self.assertTrue(_connection_token_list_is_safe(headers))

        multiple = Message()
        multiple["Connection"] = "close"
        multiple["Connection"] = "x-custom"
        self.assertTrue(_connection_token_list_is_safe(multiple))

    def test_helper_rejects_empty_elements_and_non_tokens(self):
        values = (
            "",
            " ",
            ",close",
        ...
        for value in values:
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_connection_token_list_is_safe(headers))
        self.assertFalse(_connection_token_list_is_safe({}))

    def test_valid_extension_connection_tokens_remain_accepted(self):
        ...
        try:
            response = self.raw_request(
                ...
                b"Host: localhost\r\n"
                b"Connection: x-custom, keep-alive\r\n\r\n",
            )
    ...
    def test_empty_connection_field_is_terminal_400(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_empty_connection_list_element_is_terminal_400_before_runtime(self):
    ...
    def test_quoted_and_parameterized_connection_options_are_rejected(self):
        for value in (b'"close"', b"keep-alive;timeout=5", b"close keep-alive"):
            with self.subTest(value=value):
                server = HealthServer(object(), "127.0.0.1", 0).start()
                try:
                    response = self.raw_request(
    ...
    def test_http_10_malformed_connection_list_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_malformed_connection_rejection_head_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_malformed_connection_rejection_terminates_pipeline(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_malformed_connection_releases_capacity_and_recovers(self):
        server = HealthServer(
