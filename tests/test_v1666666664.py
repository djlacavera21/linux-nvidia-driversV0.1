...
from nvlx.http_v1666666664 import HealthServer, _request_te_negotiation_is_safe
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for rejected TE negotiation")


class TENegotiationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_absent_and_non_te_connection_tokens(self):
        for value in (None, "close", "keep-alive", "team", "upgrade-token"):
            with self.subTest(value=value):
                headers = Message()
                if value is not None:
                    headers["Connection"] = value
                self.assertTrue(_request_te_negotiation_is_safe(headers))

    def test_helper_rejects_any_te_field(self):
        for value in ("trailers", "gzip", "deflate", ""):
            with self.subTest(value=value):
                headers = Message()
                headers["TE"] = value
                self.assertFalse(_request_te_negotiation_is_safe(headers))

        duplicate = Message()
        duplicate["TE"] = "trailers"
        duplicate["TE"] = "gzip"
        self.assertFalse(_request_te_negotiation_is_safe(duplicate))

    def test_helper_rejects_te_connection_token_case_insensitively(self):
        for value in ("TE", "te", "keep-alive, Te", "tE, close"):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_te_negotiation_is_safe(headers))
        self.assertFalse(_request_te_negotiation_is_safe({}))

    def test_non_te_connection_values_remain_accepted(self):
        for value in (b"close", b"team"):
            with self.subTest(value=value):
    ...
    def test_te_field_is_terminal_400_before_runtime(self):
    ...
    def test_connection_te_token_is_terminal_400(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_empty_and_duplicate_te_fields_are_rejected(self):
        payloads = (
            b"GET /livez HTTP/1.1\r\nHost: localhost\r\nTE:\r\n\r\n",
            b"GET /livez HTTP/1.1\r\nHost: localhost\r\n"
            b"TE: trailers\r\nTE: gzip\r\n\r\n",
    ...
    def test_http_10_te_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_expect_gate_retains_precedence_over_te_gate(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_trailer_gate_remains_earlier_than_te_gate(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_te_rejection_head_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_te_rejection_terminates_pipelined_bytes(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_te_rejection_releases_capacity_and_recovers(self):
        server = HealthServer(
