...
from nvlx.http_v166666663 import HealthServer, _request_expectation_is_safe
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for rejected expectations")


class ExpectContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_expectation_policy_accepts_only_absence(self):
        headers = Message()
        self.assertTrue(_request_expectation_is_safe(headers))
        headers["Host"] = "localhost"
        self.assertTrue(_request_expectation_is_safe(headers))
        headers["Expect"] = "100-continue"
        self.assertFalse(_request_expectation_is_safe(headers))
        headers["Expect"] = "extension"
        self.assertFalse(_request_expectation_is_safe(headers))
        self.assertFalse(_request_expectation_is_safe({}))
        self.assertFalse(_request_expectation_is_safe(None))

    def test_request_without_expect_remains_accepted(self):
    ...
    def test_100_continue_is_terminal_417_before_runtime(self):
    ...
    def test_empty_expect_header_is_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_duplicate_expect_headers_are_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_http_10_expectation_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_body_framing_rejection_keeps_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
            ...
            self.assertNotIn(b"417 Request Rejected", response)
        finally:
            server.close()

    def test_expect_rejection_head_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
            ...
            self.assertTrue(head.startswith(b"HTTP/1.0 417 Request Rejected\r\n"))
    ...
    def test_expect_rejection_terminates_pipelined_bytes(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_expect_rejection_releases_capacity_and_server_recovers(self):
        server = HealthServer(
            object(),
            "127.0.0.1",
            0,
