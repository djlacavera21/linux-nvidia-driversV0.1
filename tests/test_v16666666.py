...
from nvlx.http_v16666666 import HealthServer, _ObsFoldTrackingReader
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for obsolete folded headers")


class ObsFoldContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_tracking_reader_marks_only_continuation_line_starts(self):
        stream = io.BytesIO(
            b"Host: localhost\r\n"
            b"X-Test: alpha\tbeta\r\n"
            b" continuation\r\n"
            b"\tsecond-continuation\r\n"
            b"\r\n"
        )
        reader = _ObsFoldTrackingReader(stream)
        self.assertFalse(reader.saw_obs_fold)
        self.assertEqual(reader.readline(), b"Host: localhost\r\n")
        self.assertFalse(reader.saw_obs_fold)
        self.assertEqual(reader.readline(), b"X-Test: alpha\tbeta\r\n")
        self.assertFalse(reader.saw_obs_fold)
        self.assertEqual(reader.readline(), b" continuation\r\n")
        self.assertTrue(reader.saw_obs_fold)
        self.assertEqual(reader.readline(), b"\tsecond-continuation\r\n")
        self.assertTrue(reader.saw_obs_fold)

    def test_canonical_headers_remain_accepted(self):
    ...
    def test_space_folded_generic_header_is_terminal_400_before_runtime(self):
    ...
    def test_tab_folded_generic_header_is_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_http_10_folded_header_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_folded_head_rejection_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_fold_rejection_terminates_pipelined_bytes(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_fold_rejection_releases_capacity_and_server_recovers(self):
        server = HealthServer(
            object(),
            "127.0.0.1",
            0,
