...
from nvlx.http_v1666666666331 import (
    HealthServer,
    _request_websocket_metadata_is_safe,
)
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for WebSocket metadata")


class WebSocketMetadataContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_absent_and_unrelated_sec_fields(self):
        headers = Message()
        headers["Host"] = "localhost"
        headers["Sec-Fetch-Site"] = "same-origin"
        headers["Connection"] = "keep-alive"
        self.assertTrue(_request_websocket_metadata_is_safe(headers))

    def test_helper_rejects_websocket_metadata_case_insensitively(self):
        for name, value in (
            ...
            with self.subTest(name=name):
                headers = Message()
                headers[name] = value
                self.assertFalse(_request_websocket_metadata_is_safe(headers))
        self.assertFalse(_request_websocket_metadata_is_safe({}))

    def test_unrelated_sec_header_remains_accepted_live(self):
    ...
    def test_websocket_key_is_terminal_400_before_runtime(self):
    ...
    def test_http_10_websocket_metadata_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
    ...
    def test_upgrade_gate_remains_earlier(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(server, b"GET /livez HTTP/1.1\r\nHost: localhost\r\nConnection: Upgrade\r\nUpgrade: websocket\r\nSec-WebSocket-Key: abc\r\n\r\n")
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
    ...
    def test_websocket_rejection_head_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
    ...
    def test_websocket_rejection_terminates_pipeline(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(server, b"GET /livez HTTP/1.1\r\nHost: localhost\r\nSec-WebSocket-Version: 13\r\n\r\nGET /livez HTTP/1.1\r\nHost: localhost\r\n\r\n")
            self.assertEqual(response.count(b"HTTP/1.0 "), 1)
    ...
    def test_websocket_releases_capacity_and_recovers(self):
