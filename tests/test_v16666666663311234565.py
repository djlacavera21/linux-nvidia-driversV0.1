...
from nvlx.http_v16666666663311234565 import (
    HealthServer,
    _request_fastly_client_ip_nomination_is_safe,
)
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for Fastly-Client-IP-nomination requests")


class FastlyClientIpConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_fastly_client_ip_field_and_nonmatching_connection_options(self):
        for name, value in (
            (None, None),
            ("Fastly-Client-IP", "203.0.113.7"),
            ("Connection", "fastly-client-ip-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_fastly_client_ip_nomination_is_safe(headers))

    def test_helper_rejects_exact_fastly_client_ip_nomination_case_insensitively(self):
        for value in (
            "fastly-client-ip",
            "Fastly-Client-IP",
            "FaStLy-ClIeNt-Ip",
            "keep-alive, fastly-client-ip",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_fastly_client_ip_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_fastly_client_ip_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "x-cluster-client-ip"
        self.assertFalse(_request_fastly_client_ip_nomination_is_safe(headers))

    def test_ordinary_fastly_client_ip_header_remains_accepted(self):
    ...
    def test_fastly_client_ip_nomination_is_terminal_400_before_runtime(self):
        runtime = _MetricsRuntime()
        server = HealthServer(runtime, "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /metrics HTTP/1.1\r\nHost: localhost\r\nConnection: fastly-client-ip\r\n\r\n",
            )
    ...
    def test_fastly_client_ip_nomination_lookalike_remains_accepted(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_http_10_fastly_client_ip_nomination_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_x_cluster_client_ip_nomination_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\n"
                b"Connection: x-cluster-client-ip, fastly-client-ip\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
    ...
    def test_fastly_client_ip_nomination_rejection_head_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_fastly_client_ip_nomination_rejection_terminates_pipeline(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_fastly_client_ip_nomination_releases_capacity_and_recovers(self):
