...
from nvlx.http_v166666666633112345675677 import (
    HealthServer,
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for X-Forwarded-Client-Cert-nomination requests")


class XForwardedClientCertConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            (None, None),
            ("X-Forwarded-Client-Cert", 'By=spiffe://proxy;Hash=abc123;URI=spiffe://client'),
            ("Connection", "x-forwarded-client-cert-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_x_forwarded_client_cert_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            "X-Forwarded-Client-Cert",
            "X-FoRwArDeD-ClIeNt-CeRt",
            "keep-alive, x-forwarded-client-cert",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_x_forwarded_client_cert_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_x_forwarded_client_cert_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "x-envoy-ip-tags"
        self.assertFalse(_request_x_forwarded_client_cert_nomination_is_safe(headers))
    ...
    def test_ordinary_header_remains_accepted(self):
        ...
        try:
            response = self.raw_request(
                ...
                b"X-Forwarded-Client-Cert: By=spiffe://proxy;Hash=abc123;URI=spiffe://client\r\n\r\n",
            )
    ...
    def test_nomination_is_terminal_400_before_runtime(self):
        ...
        try:
            response = self.raw_request(
                server,
                b"GET /metrics HTTP/1.1\r\nHost: localhost\r\nConnection: x-forwarded-client-cert\r\n\r\n",
            )
    ...
    def test_expect_gate_retains_precedence(self):
        ...
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nExpect: 100-continue\r\n"
                b"Connection: x-forwarded-client-cert\r\n\r\n",
            )
    ...
    def test_ip_tags_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\n"
                b"Connection: x-envoy-ip-tags, x-forwarded-client-cert\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
