...
from nvlx.http_v1666666666331123 import (
    HealthServer,
    _request_forwarded_nomination_is_safe,
)
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for Forwarded-nomination requests")


class ForwardedConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_forwarded_field_and_nonmatching_connection_options(self):
        for name, value in (
            (None, None),
            ("Forwarded", "for=192.0.2.60;proto=https;host=example.com"),
            ("Connection", "forwarded-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_forwarded_nomination_is_safe(headers))

    def test_helper_rejects_exact_forwarded_nomination_case_insensitively(self):
        for value in ("forwarded", "Forwarded", "FoRwArDeD", "keep-alive, forwarded"):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_forwarded_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_forwarded_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "cookie"
        self.assertFalse(_request_forwarded_nomination_is_safe(headers))

    def test_ordinary_forwarded_header_remains_accepted(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\n"
                b"Forwarded: for=192.0.2.60;proto=https;host=example.com\r\n\r\n",
            )
    ...
    def test_forwarded_nomination_is_terminal_400_before_runtime(self):
    ...
    def test_forwarded_nomination_lookalike_remains_accepted(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_http_10_forwarded_nomination_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_cookie_nomination_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\n"
                b"Connection: cookie, forwarded\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
    ...
    def test_forwarded_nomination_rejection_head_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_forwarded_nomination_rejection_terminates_pipeline(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_forwarded_nomination_releases_capacity_and_recovers(self):
