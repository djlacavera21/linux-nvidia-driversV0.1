...
from nvlx.http_v1666666666331123456756785121 import (
    HealthServer,
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for X-Datadog-Trace-ID-nomination requests")


class XDatadogTraceIdConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            ...
            ("Connection", "x-datadog-trace-id-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_x_datadog_trace_id_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            "X-Datadog-Trace-ID",
            "X-DaTaDoG-TrAcE-Id",
            "keep-alive, x-datadog-trace-id",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_x_datadog_trace_id_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_x_datadog_trace_id_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "b3"
        self.assertFalse(_request_x_datadog_trace_id_nomination_is_safe(headers))
    ...
    def test_ordinary_header_remains_accepted(self):
        ...
        try:
            response = self.raw_request(
                ...
                b"X-Datadog-Trace-ID: 1234567890123456789\r\n\r\n",
            )
    ...
    def test_expect_gate_retains_precedence(self):
        ...
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nExpect: 100-continue\r\n"
                b"Connection: x-datadog-trace-id\r\n\r\n",
            )
    ...
    def test_b3_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\n"
                b"Connection: b3, x-datadog-trace-id\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
