...
from nvlx.http_v166666666633112341 import (
    HealthServer,
    _request_x_forwarded_port_nomination_is_safe,
)
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for X-Forwarded-Port-nomination requests")


class XForwardedPortConnectionNominationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_x_forwarded_port_field_and_nonmatching_connection_options(self):
        for name, value in (
            (None, None),
            ("X-Forwarded-Port", "443"),
            ("Connection", "x-forwarded-port-x"),
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_x_forwarded_port_nomination_is_safe(headers))

    def test_helper_rejects_exact_x_forwarded_port_nomination_case_insensitively(self):
        for value in (
            "x-forwarded-port",
            "X-Forwarded-Port",
            "X-FoRwArDeD-PoRt",
            "keep-alive, x-forwarded-port",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_x_forwarded_port_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_x_forwarded_port_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "x-forwarded-proto"
        self.assertFalse(_request_x_forwarded_port_nomination_is_safe(headers))

    def test_ordinary_x_forwarded_port_header_remains_accepted(self):
    ...
    def test_x_forwarded_port_nomination_is_terminal_400_before_runtime(self):
        runtime = _MetricsRuntime()
        server = HealthServer(runtime, "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /metrics HTTP/1.1\r\nHost: localhost\r\nConnection: x-forwarded-port\r\n\r\n",
            )
    ...
    def test_x_forwarded_port_nomination_lookalike_remains_accepted(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_http_10_x_forwarded_port_nomination_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_x_forwarded_proto_nomination_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\n"
                b"Connection: x-forwarded-proto, x-forwarded-port\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
    ...
    def test_x_forwarded_port_nomination_rejection_head_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_x_forwarded_port_nomination_rejection_terminates_pipeline(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_x_forwarded_port_nomination_releases_capacity_and_recovers(self):
