...
from nvlx.http_v16666666663311 import (
    HealthServer,
    _request_proxy_authorization_is_safe,
)
...
class _MetricsRuntime:
    ...
    def metrics_diagnosis(self):
        self.calls += 1
        raise RuntimeError("should not be reached for Proxy-Authorization requests")


class ProxyAuthorizationContainmentTests(unittest.TestCase):
    @staticmethod
    ...
    def test_helper_accepts_absent_proxy_credentials_and_normal_authorization(self):
        for name, value in ((None, None), ("Authorization", "Bearer abc"), ("Connection", "proxy-authorization-x")):
            with self.subTest(name=name, value=value):
                headers = Message()
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_proxy_authorization_is_safe(headers))

    def test_helper_rejects_any_proxy_authorization_field(self):
        for value in ("Basic Zm9vOmJhcg==", "Bearer token", ""):
            with self.subTest(value=value):
                headers = Message()
                headers["Proxy-Authorization"] = value
                self.assertFalse(_request_proxy_authorization_is_safe(headers))
        duplicated = Message()
        duplicated["Proxy-Authorization"] = "Basic one"
        duplicated["Proxy-Authorization"] = "Basic two"
        self.assertFalse(_request_proxy_authorization_is_safe(duplicated))

    def test_helper_rejects_exact_connection_nomination_case_insensitively(self):
        for value in ("proxy-authorization", "Proxy-Authorization", "keep-alive, PrOxY-AuThOrIzAtIoN"):
            with self.subTest(value=value):
                headers = Message()
    ...
    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_proxy_authorization_is_safe({}))
        headers = Message()
        headers["Sec-WebSocket-Key"] = "abc"
        self.assertFalse(_request_proxy_authorization_is_safe(headers))

    def test_normal_authorization_remains_accepted(self):
    ...
    def test_proxy_authorization_field_is_terminal_400_before_runtime(self):
    ...
    def test_empty_and_duplicate_proxy_authorization_fields_are_rejected(self):
        for fields in (
            b"Proxy-Authorization:\r\n",
            b"Proxy-Authorization: Basic one\r\nProxy-Authorization: Basic two\r\n",
        ):
    ...
    def test_exact_connection_nomination_is_rejected_but_lookalike_is_accepted(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            rejected = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nConnection: proxy-authorization\r\n\r\n",
            )
            self.assertTrue(rejected.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
            accepted = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nConnection: proxy-authorization-x\r\n\r\n",
            )
            self.assertTrue(accepted.startswith(b"HTTP/1.0 200 OK\r\n"))
        finally:
            server.close()

    def test_http_10_proxy_authorization_is_also_rejected(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_websocket_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_proxy_authorization_rejection_head_is_bodyless(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_proxy_authorization_rejection_terminates_pipeline(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
    ...
    def test_proxy_authorization_releases_capacity_and_recovers(self):
