...
from nvlx.http_v16666666663311234567567851234 import (
    HealthServer,
...
class TracestateConnectionNominationContainmentTests(unittest.TestCase):
    ...
    def test_helper_accepts_field_and_nonmatching_connection_options(self):
        for name, value in (
            ...
            with self.subTest(name=name, value=value):
                ...
                if name is not None:
                    headers[name] = value
                self.assertTrue(_request_tracestate_nomination_is_safe(headers))
    ...
    def test_helper_rejects_exact_nomination_case_insensitively(self):
        for value in (
            ...
            "Tracestate",
            "TrAcEsTaTe",
            "keep-alive, tracestate",
        ):
            with self.subTest(value=value):
                headers = Message()
                headers["Connection"] = value
                self.assertFalse(_request_tracestate_nomination_is_safe(headers))

    def test_helper_keeps_inherited_fail_closed_contract(self):
        self.assertFalse(_request_tracestate_nomination_is_safe({}))
        headers = Message()
        headers["Connection"] = "traceparent"
        self.assertFalse(_request_tracestate_nomination_is_safe(headers))
    ...
    def test_ordinary_header_remains_accepted(self):
        ...
        try:
            response = self.raw_request(
                ...
                b"Tracestate: rojo=00f067aa0ba902b7,congo=t61rcWkgMzE\r\n\r\n",
            )
    ...
    def test_traceparent_gate_retains_precedence(self):
        server = HealthServer(object(), "127.0.0.1", 0).start()
        try:
            response = self.raw_request(
                server,
                b"GET /livez HTTP/1.1\r\nHost: localhost\r\nConnection: traceparent, tracestate\r\n\r\n",
            )
            self.assertTrue(response.startswith(b"HTTP/1.0 400 Request Rejected\r\n"))
