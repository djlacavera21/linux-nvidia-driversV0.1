# nvlx: Linux-NVIDIA-Driver v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.3

`nvlx` v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.3 adds `X-Envoy-Original-Url` Connection-nomination containment to the live HTTP surface. After the inherited ingress and proxy-metadata gates succeed, the server rejects any exact `x-envoy-original-url` Connection option before endpoint or runtime evaluation while continuing to admit ordinary `X-Envoy-Original-Url` fields.

> [!IMPORTANT]
> NVIDIA driver/GPU Operator resources remain read-only. The operator still mutates only nvlx-owned GPUFleet status/finalizers plus its existing Lease and Events.

## v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.3 X-Envoy-Original-Url Connection-nomination containment

- **X-Envoy-Original-Url remains end-to-end.** A normal request `X-Envoy-Original-Url` field remains admissible when all inherited gates accept it.
- **Hop-by-hop demotion is terminal.** An exact `x-envoy-original-url` Connection option is rejected through canonical `400 Request Rejected` framing.
- **Matching is case-insensitive.** `Connection: X-Envoy-Original-Url` and mixed-case variants are rejected.
- **Substring lookalikes remain outside this rule.** Options such as `x-envoy-original-url-x` remain valid when otherwise admissible.
- **Values remain opaque.** This layer examines only Connection option names and does not interpret, log, split, validate, normalize, or trust `X-Envoy-Original-Url` values.
- **HTTP/1.0 and HTTP/1.1 are covered.** Nomination is refused under either admitted request version.
- **Earlier gates retain precedence.** `Expect`, upgrade/framing/Connection policy, credentials, forwarding/client-IP metadata, `Fastly-Client-IP`, `Fly-Client-IP`, `X-Envoy-External-Address`, `X-Envoy-Original-Dst-Host`, and `X-Envoy-Original-Path` nomination all run before this layer.
...
- **The live operator now uses `http_v166666666633112345673`.** The live runtime remains `runtime_v1664`.
...
52. HEAD rejection remains bodyless while preserving representation `Content-Length`.
53. Rejected requests cannot process trailing pipelined bytes on the same connection.
54. Rejection releases bounded worker capacity.
55. Header field-count, aggregate header bytes, and request-line byte budgets remain independently enforced.
56. Silent and byte-trickle partial requests remain bounded by the inherited idle timeout and absolute parse deadline.
57. Existing client-abort, parser-error, logging, response-body, resource, and method containment remains unchanged.
58. All v1.6.5.x checkpoint receipt, reconciliation, and persistence semantics remain unchanged.
59. NVIDIA driver/GPU Operator resources remain read-only in v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.3.
