# nvlx: Linux-NVIDIA-Driver v1.6.6.6.6.6.6.6.6.2

`nvlx` v1.6.6.6.6.6.6.6.6.2 adds protocol-upgrade containment to the live HTTP surface. After the inherited framing, version, Host, request-target, header-syntax, `Expect`, Host-authority, request-line separator, percent-escape, and canonical-CRLF gates succeed, any `Upgrade` field or any comma-delimited `Connection` token equal to `upgrade` is rejected before endpoint or runtime evaluation.

> [!IMPORTANT]
> NVIDIA driver/GPU Operator resources remain read-only. The operator still mutates only nvlx-owned GPUFleet status/finalizers plus its existing Lease and Events.

## v1.6.6.6.6.6.6.6.6.2 protocol-upgrade containment

- **The live health server never switches protocols.** Any `Upgrade` header is rejected, including empty, duplicated, WebSocket, h2c, and extension values.
- **Connection upgrade signaling is rejected token-wise.** A comma-delimited `Connection` token equal to `upgrade`, case-insensitively, is terminally rejected.
- **Substring lookalikes are not redefined.** `Connection: upgrade-token` is not treated as the `upgrade` token by this release.
- **Ordinary connection directives remain outside this gate.** `Connection: close` and `Connection: keep-alive` remain governed by inherited HTTP behavior.
- **HTTP/1.0 and HTTP/1.1 are covered.** This minimal health surface has no upgrade role under either admitted request version.
- **Earlier gates retain precedence.** Body framing, exact version admission, Host cardinality/authority, target syntax, obsolete folding, field-name/value syntax, `Expect`, request-line spacing, malformed percent escapes, and canonical CRLF line endings still run first.
...
- **Upgrade failures use canonical terminal 400 framing.** `Connection: close` on the rejection prevents trailing bytes from becoming a pipelined follow-on request.
- **HEAD rejection remains bodyless.** Representation `Content-Length` is preserved without sending the rejection body.
- **Runtime/endpoint evaluation remains isolated.** Upgrade attempts cannot invoke readiness or metrics diagnosis.
...
- **The live operator now uses `http_v1666666662`.** The live runtime remains `runtime_v1664`.
...
14. HEAD rejection remains bodyless while preserving representation `Content-Length`.
15. Rejected requests cannot process trailing pipelined bytes on the same connection.
16. Rejection releases bounded worker capacity.
17. Header field-count, aggregate header bytes, and request-line byte budgets remain independently enforced.
18. Silent and byte-trickle partial requests remain bounded by the inherited idle timeout and absolute parse deadline.
19. Existing client-abort, parser-error, logging, response-body, resource, and method containment remains unchanged.
20. All v1.6.5.x checkpoint receipt, reconciliation, and persistence semantics remain unchanged.
21. NVIDIA driver/GPU Operator resources remain read-only in v1.6.6.6.6.6.6.6.6.2.
