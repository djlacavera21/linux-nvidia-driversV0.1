# nvlx: Linux-NVIDIA-Driver v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.3

`nvlx` v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.3 adds `Traceparent` Connection-nomination containment to the live HTTP surface. After inherited ingress and tracing/proxy-metadata gates succeed, the server rejects an exact `traceparent` Connection option before endpoint/runtime evaluation while continuing to admit ordinary `Traceparent` fields.

> [!IMPORTANT]
> NVIDIA driver/GPU Operator resources remain read-only. The operator still mutates only nvlx-owned GPUFleet status/finalizers plus its existing Lease and Events.

## v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.3 Traceparent Connection-nomination containment

- **Ordinary W3C trace context remains admissible.** `Traceparent` is accepted when all inherited gates accept the request.
- **Hop-by-hop demotion is terminal.** Exact `traceparent` Connection nomination receives canonical `400 Request Rejected` framing.
- **Matching is case-insensitive.** Mixed-case exact nominations are rejected.
- **Substring lookalikes remain outside this rule.** `traceparent-x` remains valid when otherwise admissible.
- **Values remain opaque at this layer.** This containment examines only Connection option names and does not validate or reinterpret W3C trace context. Envoy documents `traceparent` as the W3C trace-context header carrying version, trace ID, parent ID, and trace flags in standardized form.
- **HTTP/1.0 and HTTP/1.1 are covered.**
- **Earlier gates retain precedence.** The complete inherited framing, credential, forwarding/client-address, Envoy timeout/retry/hedging, retriable-header/status, alt-stat, timeout-alt-response, timeout-retry-provenance, original-host, upstream-stream-duration, downstream-service-cluster/node, force-trace, IP-tags, XFCC, request-ID, client-trace-ID, OT-span-context, B3 trace-ID, B3 span-ID, B3 parent-span-ID, B3 sampled, B3 flags, compressed B3, Datadog trace-ID, Datadog parent-ID, Datadog sampling-priority, SW8, and AWS X-Ray gates run before this layer.
...
- **The live operator now uses `http_v16666666663311234567567851233`.** The live runtime remains `runtime_v1664`.
...
Protocol invariants remain fail-closed in the inherited order: request framing and target syntax; header grammar and value-octet containment; `Expect`; upgrade/Trailer/TE/Proxy-Connection; canonical Connection parsing/lifecycle/critical nomination/duplication/singleton enforcement; Keep-Alive/HTTP2-Settings/WebSocket/Proxy-Authorization; Authorization/Cookie/Forwarded and forwarding/client-IP nomination containment; Envoy external/original/internal/attempt/decorator/timeout metadata; `X-Envoy-Retry-On`; `X-Envoy-Retry-Grpc-On`; `X-Envoy-Max-Retries`; `X-Envoy-Hedge-On-Per-Try-Timeout`; `X-Envoy-Retriable-Header-Names`; `X-Envoy-Retriable-Status-Codes`; `X-Envoy-Upstream-Alt-Stat-Name`; `X-Envoy-Upstream-Rq-Timeout-Alt-Response`; `X-Envoy-Is-Timeout-Retry`; `X-Envoy-Original-Host`; `X-Envoy-Upstream-Stream-Duration-Ms`; `X-Envoy-Downstream-Service-Cluster`; `X-Envoy-Downstream-Service-Node`; `X-Envoy-Force-Trace`; `X-Envoy-IP-Tags`; `X-Forwarded-Client-Cert`; `X-Request-ID`; `X-Client-Trace-ID`; `X-OT-Span-Context`; `X-B3-TraceId`; `X-B3-SpanId`; `X-B3-ParentSpanId`; `X-B3-Sampled`; `X-B3-Flags`; `B3`; `X-Datadog-Trace-ID`; `X-Datadog-Parent-ID`; `X-Datadog-Sampling-Priority`; `SW8`; `X-Amzn-Trace-Id`; then `Traceparent`.
...
89. HEAD rejection remains bodyless while preserving representation `Content-Length`.
90. Rejected requests cannot process trailing pipelined bytes on the same connection.
91. Rejection releases bounded worker capacity.
92. Header field-count, aggregate header bytes, and request-line byte budgets remain independently enforced.
93. Silent and byte-trickle partial requests remain bounded by the inherited idle timeout and absolute parse deadline.
94. Existing client-abort, parser-error, logging, response-body, resource, and method containment remains unchanged.
95. All v1.6.5.x checkpoint receipt, reconciliation, and persistence semantics remain unchanged.
96. NVIDIA driver/GPU Operator resources remain read-only in v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.3.
