# nvlx: Linux-NVIDIA-Driver v1.6.6.6.6.6.6.6.6.6.3.3.1

`nvlx` v1.6.6.6.6.6.6.6.6.6.3.3.1 adds WebSocket handshake-metadata containment to the live HTTP surface. After the inherited framing, version, Host, request-target, header-syntax, `Expect`, Host-authority, request-line separator, percent-escape, canonical-CRLF, protocol-upgrade, `Trailer`, `TE`, `Proxy-Connection`, canonical `Connection` token-list, lifecycle-conflict, critical-nomination, duplicate-option, singleton-Connection-field, `Keep-Alive`, and `HTTP2-Settings` gates succeed, the server rejects any request header whose field name begins with `Sec-WebSocket-`, case-insensitively, before endpoint or runtime evaluation.

> [!IMPORTANT]
> NVIDIA driver/GPU Operator resources remain read-only. The operator still mutates only nvlx-owned GPUFleet status/finalizers plus its existing Lease and Events.

## v1.6.6.6.6.6.6.6.6.6.3.3.1 WebSocket handshake-metadata containment

- **WebSocket handshake metadata is terminal.** Any `Sec-WebSocket-*` request field is rejected through canonical `400 Request Rejected` framing.
- **Field-name presence alone is sufficient.** `Sec-WebSocket-Key`, `Sec-WebSocket-Version`, `Sec-WebSocket-Protocol`, `Sec-WebSocket-Extensions`, `Sec-WebSocket-Accept`, mixed-case variants, empty values, and duplicate fields are rejected without interpreting their values.
- **Unrelated Sec-* metadata remains supported.** Fields such as `Sec-Fetch-Site` are not affected by this release.
- **The inherited Upgrade gate remains earlier.** A genuine `Upgrade: websocket` request is still rejected by the existing protocol-upgrade layer before this metadata layer runs.
- **HTTP/1.0 and HTTP/1.1 are covered.** WebSocket handshake metadata is refused under either admitted request version.
- **Earlier gates retain precedence.** `Upgrade`, `HTTP2-Settings`, `Keep-Alive`, malformed Connection lists, lifecycle conflicts, critical nomination, duplicate options, repeated Connection fields, `TE`, `Proxy-Connection`, and `Expect` still run before this layer.
...
- **WebSocket metadata failures use canonical terminal 400 framing.** Rejection closes the connection so trailing bytes cannot become a pipelined follow-on request.
- **HEAD rejection remains bodyless.** Representation `Content-Length` is preserved without sending the rejection body.
- **Runtime/endpoint evaluation remains isolated.** WebSocket handshake metadata cannot invoke readiness or metrics diagnosis.
...
- **The live operator now uses `http_v1666666666331`.** The live runtime remains `runtime_v1664`.
...
25. HEAD rejection remains bodyless while preserving representation `Content-Length`.
26. Rejected requests cannot process trailing pipelined bytes on the same connection.
27. Rejection releases bounded worker capacity.
28. Header field-count, aggregate header bytes, and request-line byte budgets remain independently enforced.
29. Silent and byte-trickle partial requests remain bounded by the inherited idle timeout and absolute parse deadline.
30. Existing client-abort, parser-error, logging, response-body, resource, and method containment remains unchanged.
31. All v1.6.5.x checkpoint receipt, reconciliation, and persistence semantics remain unchanged.
32. NVIDIA driver/GPU Operator resources remain read-only in v1.6.6.6.6.6.6.6.6.6.3.3.1.
