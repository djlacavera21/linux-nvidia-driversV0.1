# nvlx: Linux-NVIDIA-Driver v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.6

`nvlx` v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.6 adds `X-Envoy-Upstream-Rq-Timeout-Alt-Response` Connection-nomination containment to the live HTTP surface. After inherited ingress and proxy-metadata gates succeed, the server rejects an exact `x-envoy-upstream-rq-timeout-alt-response` Connection option before endpoint/runtime evaluation while continuing to admit ordinary `X-Envoy-Upstream-Rq-Timeout-Alt-Response` fields.

> [!IMPORTANT]
> NVIDIA driver/GPU Operator resources remain read-only. The operator still mutates only nvlx-owned GPUFleet status/finalizers plus its existing Lease and Events.

## v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.6 X-Envoy-Upstream-Rq-Timeout-Alt-Response Connection-nomination containment

- **Ordinary timeout response metadata remains admissible.** `X-Envoy-Upstream-Rq-Timeout-Alt-Response` is accepted when all inherited gates accept the request.
- **Hop-by-hop demotion is terminal.** Exact `x-envoy-upstream-rq-timeout-alt-response` Connection nomination receives canonical `400 Request Rejected` framing.
- **Matching is case-insensitive.** Mixed-case exact nominations are rejected.
- **Substring lookalikes remain outside this rule.** `x-envoy-upstream-rq-timeout-alt-response-x` remains valid when otherwise admissible.
- **Values remain opaque at this layer.** This containment examines only Connection option names. Envoy keys this control on header presence and ignores its value; when honored, a routed request timeout returns 204 instead of 504.
- **HTTP/1.0 and HTTP/1.1 are covered.**
- **Earlier gates retain precedence.** The complete inherited framing, credential, forwarding/client-address, Envoy timeout, retry, hedge, retriable-header/status, and `X-Envoy-Upstream-Alt-Stat-Name` gates run before this layer.
...
- **The live operator now uses `http_v16666666663311234567566`.** The live runtime remains `runtime_v1664`.
...
66. HEAD rejection remains bodyless while preserving representation `Content-Length`.
67. Rejected requests cannot process trailing pipelined bytes on the same connection.
68. Rejection releases bounded worker capacity.
69. Header field-count, aggregate header bytes, and request-line byte budgets remain independently enforced.
70. Silent and byte-trickle partial requests remain bounded by the inherited idle timeout and absolute parse deadline.
71. Existing client-abort, parser-error, logging, response-body, resource, and method containment remains unchanged.
72. All v1.6.5.x checkpoint receipt, reconciliation, and persistence semantics remain unchanged.
73. NVIDIA driver/GPU Operator resources remain read-only in v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.6.
