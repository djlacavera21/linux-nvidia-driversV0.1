# nvlx: Linux-NVIDIA-Driver v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.1

`nvlx` v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.1 adds `X-Envoy-Local-Overloaded` Connection-nomination containment to the live HTTP surface. After inherited ingress, tracing, and proxy-metadata gates succeed, the server rejects an exact `x-envoy-local-overloaded` Connection option before endpoint/runtime evaluation while continuing to admit ordinary `X-Envoy-Local-Overloaded` fields.

> [!IMPORTANT]
> NVIDIA driver/GPU Operator resources remain read-only. The operator still mutates only nvlx-owned GPUFleet status/finalizers plus its existing Lease and Events.

## v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.1 X-Envoy-Local-Overloaded Connection-nomination containment

- **Ordinary fields remain admissible.** `X-Envoy-Local-Overloaded` is accepted when all inherited gates accept the request.
- **Hop-by-hop demotion is terminal.** Exact `x-envoy-local-overloaded` Connection nomination receives canonical `400 Request Rejected` framing.
- **Matching is case-insensitive.** Mixed-case exact nominations are rejected.
- **Substring lookalikes remain outside this rule.** `x-envoy-local-overloaded-x` remains valid when otherwise admissible.
- **Values remain opaque at this layer.** The containment examines only Connection option names and does not validate or reinterpret the field value.
- **Scope is request-side nomination only.** Envoy documents `x-envoy-local-overloaded` as a downstream response header it may set when overload management drops a request and the relevant configuration is enabled. nvlx does not emulate, infer, or synthesize that overload state here.
- **HTTP/1.0 and HTTP/1.1 are covered.**
- **Earlier gates retain precedence.** The complete inherited framing, credential, forwarding/client-address, Envoy timeout/retry/hedging, retriable-header/status, alt-stat, timeout-alt-response, timeout-retry-provenance, original-host, upstream-stream-duration, downstream-service-cluster/node, force-trace, IP-tags, XFCC, request-ID, client-trace-ID, OT-span-context, B3 trace-ID, B3 span-ID, B3 parent-span-ID, B3 sampled, B3 flags, compressed B3, Datadog trace-ID, Datadog parent-ID, Datadog sampling-priority, SW8, AWS X-Ray, Traceparent, and Tracestate gates run before this layer.
...
- **The live operator now uses `http_v166666666633112345675678512341`.** The live runtime remains `runtime_v1664`.
...
91. HEAD rejection remains bodyless while preserving representation `Content-Length`.
92. Rejected requests cannot process trailing pipelined bytes on the same connection.
93. Rejection releases bounded worker capacity.
94. Header field-count, aggregate header bytes, and request-line byte budgets remain independently enforced.
95. Silent and byte-trickle partial requests remain bounded by the inherited idle timeout and absolute parse deadline.
96. Existing client-abort, parser-error, logging, response-body, resource, and method containment remains unchanged.
97. All v1.6.5.x checkpoint receipt, reconciliation, and persistence semantics remain unchanged.
98. NVIDIA driver/GPU Operator resources remain read-only in v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.1.
