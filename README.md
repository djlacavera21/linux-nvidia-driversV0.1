# nvlx: Linux-NVIDIA-Driver v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4

`nvlx` v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4 adds `Tracestate` Connection-nomination containment to the live HTTP surface. After inherited ingress and tracing/proxy-metadata gates succeed, the server rejects an exact `tracestate` Connection option before endpoint/runtime evaluation while continuing to admit ordinary `Tracestate` fields.

> [!IMPORTANT]
> NVIDIA driver/GPU Operator resources remain read-only. The operator still mutates only nvlx-owned GPUFleet status/finalizers plus its existing Lease and Events.

## v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4 Tracestate Connection-nomination containment

- **Ordinary W3C trace state remains admissible.** `Tracestate` is accepted when all inherited gates accept the request.
- **Hop-by-hop demotion is terminal.** Exact `tracestate` Connection nomination receives canonical `400 Request Rejected` framing.
...
- **The live operator now uses `http_v16666666663311234567567851234`.** The live runtime remains `runtime_v1664`.
...
90. HEAD rejection remains bodyless while preserving representation `Content-Length`.
91. Rejected requests cannot process trailing pipelined bytes on the same connection.
92. Rejection releases bounded worker capacity.
93. Header field-count, aggregate header bytes, and request-line byte budgets remain independently enforced.
94. Silent and byte-trickle partial requests remain bounded by the inherited idle timeout and absolute parse deadline.
95. Existing client-abort, parser-error, logging, response-body, resource, and method containment remains unchanged.
96. All v1.6.5.x checkpoint receipt, reconciliation, and persistence semantics remain unchanged.
97. NVIDIA driver/GPU Operator resources remain read-only in v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.
