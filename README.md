# nvlx: Linux-NVIDIA-Driver v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.1

`nvlx` v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.1 adds `X-Forwarded-Port` Connection-nomination containment to the live HTTP surface. After the inherited framing, version, Host, request-target, header-syntax, `Expect`, Host-authority, request-line separator, percent-escape, canonical-CRLF, protocol-upgrade, `Trailer`, `TE`, `Proxy-Connection`, canonical `Connection` token-list, lifecycle-conflict, critical-nomination, duplicate-option, singleton-Connection-field, `Keep-Alive`, `HTTP2-Settings`, WebSocket-metadata, `Proxy-Authorization`, `Authorization` nomination, `Cookie` nomination, `Forwarded` nomination, `X-Forwarded-For` nomination, `X-Forwarded-Host` nomination, and `X-Forwarded-Proto` nomination gates succeed, the server rejects any exact `x-forwarded-port` Connection option before endpoint or runtime evaluation while continuing to admit ordinary `X-Forwarded-Port` fields.

> [!IMPORTANT]
> NVIDIA driver/GPU Operator resources remain read-only. The operator still mutates only nvlx-owned GPUFleet status/finalizers plus its existing Lease and Events.

## v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.1 X-Forwarded-Port Connection-nomination containment

- **X-Forwarded-Port remains end-to-end.** A normal request `X-Forwarded-Port` field remains admissible when all inherited gates accept it.
- **Hop-by-hop demotion is terminal.** An exact `x-forwarded-port` Connection option is rejected through canonical `400 Request Rejected` framing.
- **Matching is case-insensitive.** `Connection: X-Forwarded-Port` and mixed-case variants are rejected.
- **Substring lookalikes remain outside this rule.** Options such as `x-forwarded-port-x` remain valid when otherwise admissible.
- **X-Forwarded-Port values are opaque.** This layer examines only Connection option names and does not interpret, log, split, validate, normalize, or trust `X-Forwarded-Port` values.
- **HTTP/1.0 and HTTP/1.1 are covered.** X-Forwarded-Port nomination is refused under either admitted request version.
- **Earlier gates retain precedence.** `Expect`, `Upgrade`, `HTTP2-Settings`, WebSocket metadata, `Keep-Alive`, `Proxy-Authorization`, `Authorization` nomination, `Cookie` nomination, `Forwarded` nomination, `X-Forwarded-For` nomination, `X-Forwarded-Host` nomination, `X-Forwarded-Proto` nomination, malformed Connection lists, lifecycle conflicts, critical nomination, duplicate options, repeated Connection fields, `TE`, and `Proxy-Connection` still run before this layer.
...
- **X-Forwarded-Port-nomination failures use canonical terminal 400 framing.** Rejection closes the connection so trailing bytes cannot become a pipelined follow-on request.
- **HEAD rejection remains bodyless.** Representation `Content-Length` is preserved without sending the rejection body.
- **Runtime/endpoint evaluation remains isolated.** X-Forwarded-Port nomination cannot invoke readiness or metrics diagnosis.
...
- **The live operator now uses `http_v166666666633112341`.** The live runtime remains `runtime_v1664`.
...
33. HEAD rejection remains bodyless while preserving representation `Content-Length`.
34. Rejected requests cannot process trailing pipelined bytes on the same connection.
35. Rejection releases bounded worker capacity.
36. Header field-count, aggregate header bytes, and request-line byte budgets remain independently enforced.
37. Silent and byte-trickle partial requests remain bounded by the inherited idle timeout and absolute parse deadline.
38. Existing client-abort, parser-error, logging, response-body, resource, and method containment remains unchanged.
39. All v1.6.5.x checkpoint receipt, reconciliation, and persistence semantics remain unchanged.
40. NVIDIA driver/GPU Operator resources remain read-only in v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.1.
