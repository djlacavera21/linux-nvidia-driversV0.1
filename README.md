# nvlx: Linux-NVIDIA-Driver v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.5

`nvlx` v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.5 adds `X-Envoy-Upstream-Alt-Stat-Name` Connection-nomination containment to the live HTTP surface. After inherited ingress and proxy-metadata gates succeed, the server rejects an exact `x-envoy-upstream-alt-stat-name` Connection option before endpoint/runtime evaluation while continuing to admit ordinary `X-Envoy-Upstream-Alt-Stat-Name` fields.

> [!IMPORTANT]
> NVIDIA driver/GPU Operator resources remain read-only. The operator still mutates only nvlx-owned GPUFleet status/finalizers plus its existing Lease and Events.

## v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.5 X-Envoy-Upstream-Alt-Stat-Name Connection-nomination containment

- **Ordinary statistics metadata remains admissible.** `X-Envoy-Upstream-Alt-Stat-Name` is accepted when all inherited gates accept the request.
- **Hop-by-hop demotion is terminal.** Exact `x-envoy-upstream-alt-stat-name` Connection nomination receives canonical `400 Request Rejected` framing.
- **Matching is case-insensitive.** Mixed-case exact nominations are rejected.
- **Substring lookalikes remain outside this rule.** `x-envoy-upstream-alt-stat-name-x` remains valid when otherwise admissible.
- **Values remain opaque at this layer.** This containment examines only Connection option names. Envoy uses the request header value to emit upstream response-code and timing statistics to an additional stat tree.
- **HTTP/1.0 and HTTP/1.1 are covered.**
- **Earlier gates retain precedence.** The complete inherited framing, credential, forwarding/client-address, Envoy timeout, retry, hedge, retriable-header-name, and retriable-status-code gates run before this layer.
...
- **The live operator now uses `http_v16666666663311234567565`.** The live runtime remains `runtime_v1664`.
...
65. HEAD rejection remains bodyless while preserving representation `Content-Length`.
66. Rejected requests cannot process trailing pipelined bytes on the same connection.
67. Rejection releases bounded worker capacity.
68. Header field-count, aggregate header bytes, and request-line byte budgets remain independently enforced.
69. Silent and byte-trickle partial requests remain bounded by the inherited idle timeout and absolute parse deadline.
70. Existing client-abort, parser-error, logging, response-body, resource, and method containment remains unchanged.
71. All v1.6.5.x checkpoint receipt, reconciliation, and persistence semantics remain unchanged.
72. NVIDIA driver/GPU Operator resources remain read-only in v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.5.
