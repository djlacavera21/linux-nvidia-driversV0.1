# nvlx: Linux-NVIDIA-Driver v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.3

`nvlx` v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.3 adds `User-Agent` Connection-nomination containment to the live HTTP surface. After inherited ingress, tracing, proxy-metadata, overload-header, and Referer gates succeed, the server rejects an exact `user-agent` Connection option before endpoint/runtime evaluation while continuing to admit ordinary `User-Agent` fields.

> [!IMPORTANT]
> NVIDIA driver/GPU Operator resources remain read-only. The operator still mutates only nvlx-owned GPUFleet status/finalizers plus its existing Lease and Events.

## v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.3 User-Agent Connection-nomination containment

- **Ordinary User-Agent metadata remains admissible.** `User-Agent` is accepted when all inherited gates accept the request.
- **Hop-by-hop demotion is terminal.** Exact `user-agent` Connection nomination receives canonical `400 Request Rejected` framing.
- **Matching is case-insensitive.** Mixed-case exact nominations are rejected.
- **Substring lookalikes remain outside this rule.** `user-agent-x` remains valid when otherwise admissible.
- **Values remain opaque at this layer.** The containment examines only Connection option names and does not validate or reinterpret the User-Agent value.
- **Envoy population policy remains external.** Envoy may set `user-agent` during request decoding when `add_user_agent` is enabled and the field is absent; nvlx only prevents exact request-side hop-by-hop nomination and does not duplicate that configuration policy.
- **HTTP/1.0 and HTTP/1.1 are covered.**
- **Earlier gates retain precedence.** The complete inherited framing, credential, forwarding/client-address, Envoy timeout/retry/hedging, retriable-header/status, alt-stat, timeout-alt-response, timeout-retry-provenance, original-host, upstream-stream-duration, downstream-service-cluster/node, force-trace, IP-tags, XFCC, request-ID, client-trace-ID, OT-span-context, B3 trace-ID, B3 span-ID, B3 parent-span-ID, B3 sampled, B3 flags, compressed B3, Datadog trace-ID, Datadog parent-ID, Datadog sampling-priority, SW8, AWS X-Ray, Traceparent, Tracestate, X-Envoy-Local-Overloaded, and Referer gates run before this layer.
...
- **The live operator now uses `http_v166666666633112345675678512343`.** The live runtime remains `runtime_v1664`.
...
93. HEAD rejection remains bodyless while preserving representation `Content-Length`.
94. Rejected requests cannot process trailing pipelined bytes on the same connection.
95. Rejection releases bounded worker capacity.
96. Header field-count, aggregate header bytes, and request-line byte budgets remain independently enforced.
97. Silent and byte-trickle partial requests remain bounded by the inherited idle timeout and absolute parse deadline.
98. Existing client-abort, parser-error, logging, response-body, resource, and method containment remains unchanged.
99. All v1.6.5.x checkpoint receipt, reconciliation, and persistence semantics remain unchanged.
100. NVIDIA driver/GPU Operator resources remain read-only in v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.3.
