# nvlx: Linux-NVIDIA-Driver v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.5.4

`nvlx` v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.5.4 adds `X-Envoy-OAuth-Failure-Reason` Connection-nomination containment to the live HTTP surface. After inherited ingress, tracing, proxy-metadata, overload-header, Referer, User-Agent, original-method, peer-metadata, authorization-body, and OAuth-status gates succeed, the server rejects an exact `x-envoy-oauth-failure-reason` Connection option before endpoint/runtime evaluation while continuing to admit ordinary `X-Envoy-OAuth-Failure-Reason` fields.

> [!IMPORTANT]
> NVIDIA driver/GPU Operator resources remain read-only. The operator still mutates only nvlx-owned GPUFleet status/finalizers plus its existing Lease and Events.

## v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.5.4 X-Envoy-OAuth-Failure-Reason Connection-nomination containment

- **Ordinary OAuth failure-reason metadata remains admissible.** `X-Envoy-OAuth-Failure-Reason` is accepted when all inherited gates accept the request.
- **Hop-by-hop demotion is terminal.** Exact `x-envoy-oauth-failure-reason` Connection nomination receives canonical `400 Request Rejected` framing.
- **Matching is case-insensitive.** Mixed-case exact nominations are rejected.
- **Substring lookalikes remain outside this rule.** `x-envoy-oauth-failure-reason-x` remains valid when otherwise admissible.
- **Values remain opaque at this layer.** The containment examines only Connection option names and does not validate or reinterpret OAuth failure-reason metadata.
- **Envoy OAuth2 semantics remain external.** When `allow_failed_matcher` permits a request despite failed OAuth validation, Envoy's OAuth2 filter strips OAuth cookies and adds `x-envoy-oauth-status: failed` plus `x-envoy-oauth-failure-reason` before forwarding upstream. nvlx only prevents exact request-side hop-by-hop nomination and does not duplicate OAuth validation or policy.
- **HTTP/1.0 and HTTP/1.1 are covered.**
- **Earlier gates retain precedence.** The complete inherited framing, credential, forwarding/client-address, Envoy timeout/retry/hedging, retriable-header/status, alt-stat, timeout-alt-response, timeout-retry-provenance, original-host, upstream-stream-duration, downstream-service-cluster/node, force-trace, IP-tags, XFCC, request-ID, client-trace-ID, OT-span-context, B3 trace-ID, B3 span-ID, B3 parent-span-ID, B3 sampled, B3 flags, compressed B3, Datadog trace-ID, Datadog parent-ID, Datadog sampling-priority, SW8, AWS X-Ray, Traceparent, Tracestate, X-Envoy-Local-Overloaded, Referer, User-Agent, X-Envoy-Original-Method, X-Envoy-Peer-Metadata, X-Envoy-Peer-Metadata-ID, X-Envoy-Auth-Partial-Body, and X-Envoy-OAuth-Status gates run before this layer.
...
- **The live operator now uses `http_v16666666663311234567567851234454`.** The live runtime remains `runtime_v1664`.
...
99. HEAD rejection remains bodyless while preserving representation `Content-Length`.
100. Rejected requests cannot process trailing pipelined bytes on the same connection.
101. Rejection releases bounded worker capacity.
102. Header field-count, aggregate header bytes, and request-line byte budgets remain independently enforced.
103. Silent and byte-trickle partial requests remain bounded by the inherited idle timeout and absolute parse deadline.
104. Existing client-abort, parser-error, logging, response-body, resource, and method containment remains unchanged.
105. All v1.6.5.x checkpoint receipt, reconciliation, and persistence semantics remain unchanged.
106. NVIDIA driver/GPU Operator resources remain read-only in v1.6.6.6.6.6.6.6.6.6.3.3.1.2.3.4.5.6.7.5.6.7.8.5.1.2.3.4.5.4.
