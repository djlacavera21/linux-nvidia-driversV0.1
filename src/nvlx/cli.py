...
def cmd_plan(_):
    ...
    print("Safety:                install creates package + module snapshots and arms boot validation\n")
...
def cmd_install(a):
    ...
    print("Regenerate initramfs if required and reboot; nvlx boot-validate checks the transaction after boot.")
    return 0
...
def cmd_session(_):
    report=session_report(); _json(report.to_dict()); return 2 if report.warnings else 0
def cmd_topology(_):
    report=topology_report(); _json(report.to_dict()); return 0 if report.available else 1
def cmd_mig(_):
    report=mig_fabric_report(); _json(report.to_dict()); return 2 if report.fabric_manager_aligned is False or report.dcgm_compatible is False else 0
def cmd_health(a):
    report=health_report(require_expected_version=not a.allow_other_version); _json(report.to_dict()); return 0 if report.healthy else 2
...
def cmd_boot_guard(a):
    path=install_boot_guard(confirmed=a.yes); print(f"Installed and enabled boot guard: {path}"); return 0
...
def build_parser():
    p=argparse.ArgumentParser(prog="nvlx",description="Linux NVIDIA driver build, transaction, health, and diagnostics toolkit"); p.add_argument("--version",action="version",version=f"%(prog)s {__version__}"); sub=p.add_subparsers(dest="command",required=True)
    ...
    sub.add_parser("package-state").set_defaults(func=cmd_packages); sub.add_parser("transaction-pending").set_defaults(func=cmd_pending)
    q=sub.add_parser("boot-validate"); q.add_argument("--auto-rollback",action="store_true"); q.set_defaults(func=cmd_boot_validate)
    q=sub.add_parser("boot-guard-install"); q.add_argument("--yes",action="store_true"); q.set_defaults(func=cmd_boot_guard)
