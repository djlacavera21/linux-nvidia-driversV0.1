...
def cmd_plan(_):
    ...
    print('Safety:                install creates a rollback snapshot before touching kernel modules\n')
...
def cmd_install(a):
    c=load_driver_config(); sid=install_modules(Path(a.source),c,confirmed=a.yes,jobs=a.jobs,rollback_root=Path(a.rollback_root) if a.rollback_root else None); print(f'Pre-install rollback snapshot: {sid}'); print(f'Installed NVIDIA kernel modules for release {c.version}; regenerate initramfs if required, then reboot or perform a controlled module transition'); return 0
...
def build_parser():
    p=argparse.ArgumentParser(prog='nvlx',description='Linux NVIDIA driver build and diagnostics toolkit'); p.add_argument('--version',action='version',version=f'%(prog)s {__version__}'); sub=p.add_subparsers(dest='command',required=True)
    ...
    q=sub.add_parser('install'); q.add_argument('--source',required=True); q.add_argument('--jobs',type=int); q.add_argument('--rollback-root'); q.add_argument('--yes',action='store_true'); q.set_defaults(func=cmd_install)
